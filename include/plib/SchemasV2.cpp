...
public:
    ...
    void _addNodeAtts(vector<string> tokens, int pos){
        int size = tokens.size(), att = 0, actualType = nodes[pos].id;
        string  s = "-";
        for (int i = 1; i < size; i++) {
            if ((tokens[i] != s) & (tokens[i] != "")) {
                nodes[actualType].attributes[att]->addAtt
                        (tokens[i], wtSimbols.rank(wtSimbols.size(), actualType), 0);
                printf("Token %s\n",tokens[i].c_str());
            }
            att++;
        }
    ...
    int add(vector<string> info, vector<int> attType){
        if (info.size() == 0)
            return -1;
        int pos = binarySearch(info[0],nodes);
        if (pos == nodes.size() || info[0].compare(nodes[pos].labelName)!=0) {
            printf("New type required\n");
            node nod = node();
            nod.labelName = info[0];
            for (int i = 0; i < attType.size(); i++)
                if (attType[i] == -1)
                    nod.attributes.push_back(NULL);
                else
                    (attType[i]==0)?nod.attributes.push_back(new DenseAtt()):
                                nod.attributes.push_back(new SparseAtt());
            int numNodes = (!this->id)?-2:0;
            nod.id = this->id++;
            nod.numNodes = numNodes;
            (pos == nodes.size())?nodes.push_back(nod):
                        (void)nodes.insert(nodes.begin()+pos+info[0].
                                            compare(nodes[pos].labelName), nod);
            pos = (pos == nodes.size())?pos:pos+info[0].compare(nodes[pos].labelName);
            for (int i = 0; i < nodes.size(); i++)
                printf("Labels %s\n",nodes[i].labelName.c_str());
        }
        printf("Adding in %s\n",nodes[pos].labelName.c_str());
        _addNodeAtts(info,pos);
        return 1;
    }
